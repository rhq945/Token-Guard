...
try:
    ...
    OpenAI = None
    print("Warning: 'openai' module not found. API features will not work.")
# --- [1] 仅使用 Transformers，移除 VLLM ---
...
from token_guard_plugin import TokenGuardScorer, TokenGuardConfig, SegmentArtifact
...
TEMPERATURE = 1.0  # Temperature parameter for softmax sampling
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Phi-Decoding Algorithm (Single Model HF)")
    ...
    # === [新增] 在 return parser.parse_args() 之前添加以下几行 ===
    parser.add_argument('--use_api', action='store_true', help='Use OpenAI API for generation')
    parser.add_argument('--api_key', type=str, default='sk-zk2728fd09b15b325dd68f9d53ff596ce47692efdecae757', help='API Key')
    parser.add_argument('--api_base_url', type=str, default='https://api.zhizengzeng.com/v1', help='API Base URL')
    parser.add_argument('--api_model_name', type=str, default='gpt-4o-mini', help='API Model Name')
...
class PhiDecoder:
    ...
    def initialize_model(self):
        """
        Initialize the model using TokenGuardScorer to allow SHARED MEMORY.
        ...
        self.tg_scorer = TokenGuardScorer(
        ...
        if self.args.use_api:
            print(f"[Generator] Mode: API ({self.args.api_model_name})")
            if OpenAI is None:
                raise ImportError("Run `pip install openai` first.")
            
            self.client = OpenAI(
                api_key=self.args.api_key,
                base_url=self.args.api_base_url
            )
            # 注意：使用API时，self.model 指向本地模型仅用于 TokenGuard 内部计算
            self.model = self.tg_scorer.model 
        else:
            print(f"[Generator] Mode: Local HuggingFace")
            self.model = self.tg_scorer.model
            self.model.config.pad_token_id = self.tokenizer.eos_token_id
...
# === [新增函数] API 生成器 ===
# === [修复版] API 生成专用函数 ===
    def _api_generate(self, prompts: List[str], n_return: int, stop_strs: List[str] = None) -> Tuple[List[str], List[float]]:
        """
        使用 GPT-4o-mini (或其他 OpenAI 兼容 API) 生成文本。
        修复：增加了 stop_strs 参数以支持停止词。
        """
        responses = []
        cumulative_logprobs = []
        ...
        for prompt in prompts:
            try:
                ...
                completion = self.client.chat.completions.create(
                    model=self.args.api_model_name, # 使用参数指定的模型名
                    messages=messages,
                    n=n_return,
                    temperature=TEMPERATURE,
                    max_tokens=1024,
                    logprobs=True,      # 必须开启 logprobs 用于计算置信度
                    top_logprobs=1,
                    stop=stop_strs      # [关键修复] 传递停止词给 API
                )
                ...
                for choice in completion.choices:
                    ...
                    # 计算平均 Logprob (作为 Advantage 的依据)
                    if choice.logprobs and choice.logprobs.content:
                        token_logprobs = [t.logprob for t in choice.logprobs.content]
                        # 避免除以 0
                        avg_logprob = sum(token_logprobs) / (len(token_logprobs) + 1e-8)
                        cumulative_logprobs.append(avg_logprob)
                    else:
                        # 如果 API 没返回 logprobs，给个默认值
                        cumulative_logprobs.append(0.0)
                ...
                for _ in range(n_return):
                    responses.append("")
                    cumulative_logprobs.append(-10.0) 
                    
        return responses, cumulative_logprobs
    # === [关键新增] Helper function to simulate VLLM generation using HF ===
    def _hf_generate(self, prompts: List[str], n_return: int, max_new_tokens: int = 1024, stop_strs: List[str] = None) -> Tuple[List[str], List[float]]:
        """
    ...
    def process_example(self, example, system_prompt):
        ...
        # 3. Initialize Semantic History (List[List[SegmentArtifact]])
    ...
    def _process_step(self, example, system_prompt, previous_steps, previous_values, 
            ...
            # raw_responses, raw_logprobs = self._hf_generate(
            #     all_prompts, 
            ...
            if self.args.use_api:
                # ---> 走 API 通道
                raw_responses, raw_logprobs = self._api_generate(
                    all_prompts, 
                    n_return=self.args.num_rollout,
                    stop_strs=["\n", "<end_of_reasoning>"]
                )
            ...
            if self.args.width_pruning_strategy != "none" and self.args.width_pruning_strategy != "":
                ...
                # In HF single-pass mode, we assume first stage IS the completed response
                completed_responses = filtered_responses
                completed_logprobs = filtered_logprobs
                completed_advantages = filtered_advantages

                print(f"\n=== Filtered Responses (Total: {len(filtered_responses)}) ===")
                print("=" * 50)

            # =================================================================
            # === [Token-Guard] Verification & Global Scoring Phase ===
            # =================================================================
            ...
            try:
                ...
                if clusters is None:
                    ...
                    cluster_list = [sorted(clusters[i]) for i in range(self.args.cluster_num)]
                    # Calculate weights
    ...
    def _generate_final_response(self, example, system_prompt, previous_steps, previous_values, token_stats, rollout_stats, traj_info):
            ...
            # === [修改] 根据参数选择生成方式 ===
            if self.args.use_api:
                # 使用 API 生成 (注意：API生成无需 max_new_tokens 参数，或者在内部处理了)
                responses, logprobs = self._api_generate(
                    all_prompts, 
                    n_return=1,
                ...
                # 使用本地 HF 模型生成
                responses, logprobs = self._hf_generate(
                    all_prompts, 
                    n_return=1, 
                    max_new_tokens=3000, 
                    stop_strs=["<end_of_reasoning>"]
